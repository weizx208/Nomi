...
每轮由 hook（`self-check.sh`，L0）自动顶在眼前，不在此复述。一句话：**动手写码前**想清楚(P5)——用户可见先读设计系统+出可体验样张+拍板(R8)、碰三方库先 Context7(R5)、多文件先 `docs/plan`(R4)、取舍给对比表(R3)；**报完成前**全绿≠完成(P3)——和样张对账+真机走查(R13)；**push 前**五门全过(R11)。贯穿：根因不症状(P2)、加新删旧无并行版(P1)、derive 不 hardcode、分层≤800 行(R9)。细节查 `docs/engineering-rules.md`。
